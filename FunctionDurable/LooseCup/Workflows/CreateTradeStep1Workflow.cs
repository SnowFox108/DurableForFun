...
namespace FunctionDurable.LooseCup.Workflows
{
    public static class CreateTradeStep1Workflow
    {
            ...
            ILogger log)
        {
            ...
            log.LogInformation($"Starting the Step1SaveToDatabase Activity {context.InstanceId} for Trade {tradeName}");
            var trade = await context.CallActivityAsync<Trade>(nameof(Step1SaveToDatabaseActivity.SaveToDatabase), tradeName);

            log.LogInformation($"Starting the Step1Utilisation Activity {context.InstanceId} for Trade {tradeName}");
